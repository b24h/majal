...
export default function Discounts() {
  ...
  useEffect(() => {
    const axiosDiscounts = async () => {
      try {
        const res = await axios.get("/data/discounts.json");
        const withStatus = res.data.map((d) => ({
          ...d,
          status: computeStatus(d.startDate, d.endDate),
        }));
        setDiscounts(withStatus);
      } catch (err) {
        console.error("Erreur axios discounts:", err);
      }
    };
    axiosDiscounts();
  }, []);
  ...
  const openAdd = () => {
    ...
    setForm({ title: "", percentage: "", product: "", startDate: "", endDate: "" });
  ...
  const saveDiscount = () => {
    if (!form.title || !form.percentage || !form.startDate || !form.endDate) return;
    const status = computeStatus(form.startDate, form.endDate);

    if (editingDiscount) {
      setDiscounts((prev) =>
        prev.map((d) =>
          d.id === editingDiscount.id ? { ...d, ...form, status } : d
        )
      );
    } else {
      setDiscounts((prev) => [...prev, { id: Date.now(), ...form, status }]);
    }
            ...
            >
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="text-sm mt-1 text-gray-500 font-semibold">{d.product}</p>
                </div>
