...
import {LayoutDashboard, Package, Store, Tag, Percent, ShoppingBag, LogOut, Layers, Users, Gift, Bell, ChevronDown, TrendingUp, ClipboardList, DollarSign} from "lucide-react";
    ...
    return(
        <div className={`fixed left-0 top-0 h-screen bg-cyan-900 text-white flex flex-col transition-all duration-300 ${
            ...
            <div className="flex items-center justify-between px-4 py-4 border-b border-gray-300">
                <h1 className={`font-semibold text-lg tracking-wide ${collapsed ? "hidden" : "block"}`}>
                    Espace Admin
                </h1>
                <button onClick={() => setCollapsed(!collapsed)} className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-white/10 transition">
                    {collapsed ? "⫸" : "⫷"}
                </button>
            </div>
            ...
            <nav className="sidebar-scroll flex-1 px-3 py-6 space-y-3 overflow-y-auto">
                <SidebarGroup
                    ...
                    <SidebarLink to="/dashboard/ventes" icon={<TrendingUp size={20}/>} label="Ventes" collapsed={collapsed}/>
                    <SidebarLink to="/dashboard/commandes" icon={<ClipboardList size={20}/>} label="Commandes" collapsed={collapsed}/>
                    <SidebarLink to="/dashboard/revenues" icon={<DollarSign size={20}/>} label="Revenues" collapsed={collapsed}/>
                </SidebarGroup>
                ...
                <SidebarGroup
                    ...
                    toggle={() =>setProductsOpen(!productsOpen)}>
                        <div className={`${collapsed ? "hidden" : "ml-8 space-y-2"}`}>
                            <SidebarLink to="/products" icon={<Store size={20}/>} label="Liste des produits" collapsed={collapsed}/>
                            ...
                            <SidebarLink to="/packs" icon={<Tag size={20}/>} label="Packs" collapsed={collapsed}/>
                        </div>
                </SidebarGroup>
...
function SidebarGroup({icon, label, collapsed, open, toggle, children}){
    return(
        <div>
            <button onClick={toggle} className="flex items-center justify-between w-full py-2 px-2 rounded-lg hover:bg-cyan-600 transition">
