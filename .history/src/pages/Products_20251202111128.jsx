          ...
          <div className="bg-white rounded-2xl shadow-lg w-full max-w-md max-h-[90vh] flex flex-col p-6 relative">
            <div className="bg-cyan-800 text-white py-3 px-4 rounded-t-2xl -mx-6 -mt-6 mb-4 text-lg font-semibold">
              ...
              <div className="px-6 pb-6 overflow-y-auto">
                ...
                <input type="file" accept="image/*" onChange={(e) => {const file = e.target.files[0]; if(file){const url = URL.createObjectURL(file); setModalData({...modalData, img:url});}}} className="mt-2" />
              <div>
                <label className="text-sm font-medium text-gray-700">Nom</label>
                <input type="text" value={modalData?.name || ""} onChange={(e) => setModalData({ ...modalData, name: e.target.value })} className="mt-1 w-full border rounded-lg p-2"/>
              </div>
              <select value={modalData.coop} onChange={(e) =>setModalData({...modalData, coop: e.target.value})} className="p-2 border rounded">
                <option>Choisir une coopérative</option>
                {cooperatives.map((c) => (
                  <option key={c}>{c}</option>
                ))}
              </select>
              <select value={modalData.categ} onChange={(e) => setModalData({...modalData, categ: e.target.value})} className="p-2 border rounded">
