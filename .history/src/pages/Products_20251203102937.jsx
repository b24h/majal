  ...
  return (
    <div className="relative p-6 font-poppins">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {filtered.map((p) => (
                <div key={p.id} className="bg-white rounded-2xl p-4 shadow border border-gray-200 flex flex-col">
                  ...
                  <div className="mt-4 flex-1">
                    ...
                    <p className="text-sm text-gray-600 mt-1 line-clamp-2">{p.coop}</p>
                    <p className="text-sm text-gray-600 mt-1 line-clamp-2">{p.categ}</p>
      
                    <div className="mt-3 flex items-center justify-between">
                      <div>
                        <p className="font-bold">{p.price} MAD</p>
                        <p className="text-sm text-gray-600 mt-1 line-clamp-2">• {p.status}</p>
                      </div>
                      <div className="flex items-center gap-2">
                        <EditButton onClick={() => openEditModal(p)}/>
                        <DeleteButton onClick={() => showDeleteModal(p)}/>
                      </div>
                    </div>
                  </div>
                </div>
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 px-4">
          <div className="bg-white rounded-2xl shadow-lg w-full max-w-sm p-6 text-center">
            <h3 className="text-lg font-semibold text-gray-800 mb-4">Supprimer ce produit ?</h3>
            <p className="text-gray-500 mb-6">
              Êtes-vous sûr de vouloir supprimer <span className="font-medium text-gray-800">{selectedProduct?.name}</span> ?
            </p>
    ...
    <div className="bg-white rounded-2xl shadow-lg w-full max-w-md max-h-[90vh] flex flex-col p-6 relative">
      ...
      <div className="px-6 pb-6 overflow-y-auto">
        ...
        <select
          ...
          onChange={(e) => setModalData({ ...modalData, coop: e.target.value })}
        ...
        <select
          ...
          onChange={(e) => setModalData({ ...modalData, categ: e.target.value })}
          className="p-2 border rounded mt-4 w-full"
        >
        ...
        <div className="mt-4">
          ...
          <input
            ...
            onChange={(e) => setModalData({ ...modalData, quant: e.target.value })}
