    ...
    return(
        <div className={`fixed left-0 top-0 h-screen bg-cyan-900 text-white flex flex-col transition-all duration-300 ${
            ...
            <nav className="sidebar-scroll flex-1 px-3 py-6 space-y-3 overflow-y-auto">
                ...
                >
                    <SidebarLink to="/dashboard/ventes" collapsed={collapsed} icon={<TrendingUp size={18}/>} label="Ventes" collapsed={collapsed}/>
                    <SidebarLink to="/dashboard/commandes" icon={<ClipboardList size={18}/>} label="Commandes" collapsed={collapsed}/>
                    <SidebarLink to="/dashboard/revenues" icon={<DollarSign size={18}/>} label="Revenues" collapsed={collapsed}/>
                </SidebarGroup>
                ...
                <SidebarGroup
                    ...
                    toggle={() =>setProductsOpen(!productsOpen)}>
                        <SidebarLink to="/products" icon={<Store size={18}/>} label="Liste des produits" collapsed={collapsed}/>
...
function SidebarLink({to, icon, label, collapsed}){
    return(
        <Link to={to} className="flex items-center justify-between w-full py-2 px-2 rounded-lg hover:bg-cyan-600 transition">
            <div className="flex items-center gap-3">
                {icon && icon}
                {!collapsed && <span className="font-medium">{label}</span>}
            </div>
