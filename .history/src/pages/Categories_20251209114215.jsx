...
export default function Categories() {
  ...
  useEffect(() => {
    const loadData = async () => {{
      try {
        const [categorieRes, productRes] = await Promise.all([])
      }
    }
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 px-4">
          <div className="bg-white rounded-2xl shadow-lg w-full max-w-sm p-6 text-center">
            <h3 className="text-lg font-semibold text-gray-800 mb-4">Supprimer cette catégorie ?</h3>
            <p className="text-gray-500 mb-6">
              Êtes-vous sûr de vouloir supprimer <span className="font-medium text-gray-800">{categoryToDelete?.name}</span> ?
            </p>
