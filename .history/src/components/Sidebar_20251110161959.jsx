...
export default function Sidebar(){
    ...
    return(
        <div className={`${ collapsed ? "w-20" : "w-64"} bg-green-900 text-white h-screen flex flex-col transition-all duration-300`}>
            <div className="flex items-center justify-between px-4 py-4 border-b border-indigo-500">
                ...
                <button onClick={() => setCollapsed(!collapsed)} className="text-green-600 hover:text-white">
                    {collapsed ? "⫸" : "⫸"}
                </button>
            </div>
...
function SidebarLink({to, icon, label, collapsed}){
    return(
        <Link to={to} className="flex items-center gap-3 py-2 rounded-lg hover:bg-green-600 transition">
            {icon}
