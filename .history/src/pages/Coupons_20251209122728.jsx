...
export default function Coupons() {
  ...
  useEffect(() => {
    axios.get("/data/coupons.json")
      .then(res => setCoupons(res.data))
      .catch(err => console.error("Erreur axios coupons:", err));
  }, []);
      ...
      <div className="space-y-6">
        {filteredCoupons.length === 0 ? (
          <div className="bg-white rounded-2xl p-8 shadow-md text-center border border-dashed border-gray-200">
            ...
            <button
              onClick={() => setEditingIndex(null) || setFormOpen(true)}
          ...
          filteredCoupons.map((c, i) => (
            <div
              ...
              className={`rounded-xl shadow-sm p-6 flex flex-col gap-3 transition ${
                getStatus(c) === "Expirés" ? "bg-gray-200 opacity-70" : "bg-gray-50"
              }`}
            >
                ...
                <div>
                  <h3
                    className={`text-lg font-extrabold tracking-wider uppercase ${
                      getStatus(c) === "Expirés" ? "text-gray-500" : ""
                    }`}
                  >
                    {c.code} - {c.percentage}%
                  </h3>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 px-4">
          <div className="bg-white rounded-2xl shadow-lg w-full max-w-sm p-6 text-center">
            ...
            <p className="text-gray-500 mb-6">
              Êtes-vous sûr de vouloir supprimer <span className="font-medium text-gray-800">{coupons[deleteIndex]?.code}</span> ?
            </p>
