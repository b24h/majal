...
export default function Sidebar(){
    ...
    return(
        <div className={`${ collapsed ? "w-20" : "w-64"} bg-cyan-900 text-white h-screen flex flex-col transition-all duration-300 fixed`}>
            <div className="flex items-center justify-between px-4 py-4 border-b border-cyan-900">
                <h1 className={`font-bold text-x1 ${collapsed ? "hidden" : "block"}`}>
                    Majal Admin
                </h1>
                <button onClick={() => setCollapsed(!collapsed)} className="text-cyan-900 hover:text-white">
                    {collapsed ? "⫸" : "⫸"}
                </button>
            </div>
