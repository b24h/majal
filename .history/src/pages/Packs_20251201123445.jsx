...
const products = [
  ...
  return (
    <div className="p-6">
      ...
      {showEditModal && (
        <Modal show={showEditModal} title={editingPack?.id ? "Modifier le pack" : "Ajouter un pack"} onClose={() => {setShowEditModal(false); setEditingPack(null); setPreviewImage(null);}} onSave={handleSave}>
          <div className="px-6 pb-6 overflow-y-auto">
            <label className="text-sm font-medium text-gray-700">Image du pack</label>
            <input type="file" accept="image/*" onChange={handleFileChange} className="mt-2" />

            <label className="text-sm font-medium text-gray-700">Nom :</label>
            <input type="text" value={editingPack?.name || ""} onChange={(e) => setEditingPack((p) =>({...p, name: e.target.value}))} className="mt-1 w-full border rounded-lg p-2" />

            <label className="text-sm font-medium text-gray-700">Description</label>
            <textearea value={editingPack?.description || ""} onChange={(e) => setEditingPack((p) => ({...p, description: e.target.value}))} className="mt-1 w-full border rounded-lg p-2 mt-1" rows={3}/>
            ...
            <input type="number" min="0" max="90" value={editingPack.reduction} onChange={(e) => setEditingPack({...editingPack, reduction: Number(e.target.value)})} className="w-full" />
          </div>
        </Modal>
      )}
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 px-4">
          <div className="bg-white rounded-2xl shadow-lg w-full max-w-md max-h-[90vh] flex flex-col p-6 relative">
            <div className="bg-cyan-800 text-white py-3 px-4 rounded-t-2xl -mx-6 -mt-6 mb-4 text-lg font-semibold">
              ...
              <label className="text-sm font-medium text-gray-700 mt-4">Taux de réduction (%)</label>
              <input type="number" min="0" max="90" value={editingPack.reduction} onChange={(e) => setEditingPack({...editingPack, reduction: Number(e.target.value)})}
