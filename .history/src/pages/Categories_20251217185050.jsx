...
export default function Categories() {
  ...
  // const categoryToDelete = Array.isArray(categories) ? categories.find(c => c.id === pendingDeletedId) : null;
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const data = Array.isArray(categoriesRes.data)
          ? categoriesRes.data
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (editingCategory) {
        await categoriesService.updateCategory(editingCategory.id, catData);
        ...
        const res = await categoriesService.createCategory(catData);
  ...
  const handleDelete = async () => {
    ...
    try{
      await categoriesService.deleteCategory(pendingDeletedId);
  ...
  const getImageSrc = (value, defaultType = "image/png") => {
    ...
    return `data:${defaultType};base64,${value}`;
  }
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredCategories.map((cat) => (
            ...
            >
              <div>
                {cat.icon && (
                  <img src={getImageSrc(cat.icon, "image/png")} alt="icon" className="w-10 h-10 mb-3 object-contain" />
                )}
                {cat.photo && (
                  <img src={getImageSrc(cat.photo, "image.jpeg"):image/jpeg;base64,${cat.photo}`} alt="photo" className="w-full h-32 object-cover rounded-xl mb-3" />
                )}
