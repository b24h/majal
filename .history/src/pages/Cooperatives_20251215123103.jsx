...
export default function Cooperatives() {
  ...
  const handleSave = async () => {
    if (!coopData.name) return;
    try{
      if(editingCoop){
        await cooperativesService.updateCooperative(editingCoop.id, coopData);

        setCooperatives(prev =>
          prev.map(c => c.id === editingCoop.id ? {...c, ...coopData} : c)
        );
      }else{
        const res = await cooperativesService.createCooperative(...coopData);
  ...
  const confirmDelete = async () => {
    try {
      await cooperativesService.deleteCooperative(coopToDelete.id);
          ...
          {filteredCoops.map((coop) => (
            ...
            >
              <div>
                {coop.photo && (
                  <img
                    src={coop.photo`data:image/png;base64,${coop.photo}`}
                    alt={coop.name}
            ...
            <div className="px-6 pb-6 overflow-y-auto">
              ...
              <div>
                ...
                <textarea
                  ...
                  value={coopData.longDesc}
                  onChange={(e) =>
                    setCoopData({...coopData, longDesc: e.target.value})}
                  className="mt-1 w-full border rounded-lg p-2"/>
              </div>
              ...
              <div>
                ...
                <input
                  type="text"
                  placeholder="Vision"
                  value={coopData.vision}
                  onChange={(e) =>
                    setCoopData({ ...coopData, vision: e.target.value })
                  }
                  className="mt-1 w-full border rounded-lg p-2"
                />
              </div>
              ...
              <div>
                ...
                <input
                  type="text" 
                  value={coopData.photo} 
                  onChange={(e) => setCoopData({...coopData, photo: e.target.value})}
                  className="mt-1 w-full border rounded-lg p-2" />
              </div>
              ...
              <div>
                ...
                <input
                  type="text" 
                  value={coopData.videoURL} 
                  onChange={(e) => setCoopData({...coopData, videoURL: e.target.value})}
                  className="mt-1 w-full border rounded-lg p-2" />
              </div>
