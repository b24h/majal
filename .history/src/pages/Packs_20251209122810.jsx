...
export default function Packs() {
  ...
  useEffect(() => {
    const axiosPacks = async () => {
      try {
        const [productsRes, packsRes] = await Promise.all([
          axios.get("/data/products.json"),
          axios.get("/data/packs.json"),
        ]);
        setProducts(productsRes.data);
        setPacks(packsRes.data);
      } catch (err) {
        console.error("Erreur axios packs:", err);
      }
    };
    axiosPacks();
  }, []);
  ...
  useEffect(() => {
    ...
    const subtotal = (editingPack.products || []).reduce((sum, id) => {
    ...
    setEditingPack((prev) => ({ ...prev, price: finalPrice }));
  }, [editingPack?.products, editingPack?.reduction, products]);
  ...
  const handleFileChange = (e) => {
    ...
    setPreviewImage(url);
    setEditingPack((prev) => ({ ...prev, image: url, _file: file }));
  };
  ...
  const handleSave = () => {
    if (!editingPack?.name || editingPack.products.length === 0) {
      alert("Veuillez renseigner le nom du pack et sélectionner des produits.");
      return;
    }

    if (editingPack.id) {
      setPacks((prev) =>
        prev.map((p) => (p.id === editingPack.id ? { ...editingPack } : p))
      );
    } else {
      setPacks((prev) => [
        ...prev,
        { ...editingPack, id: Date.now(), image: editingPack.image || null },
      ]);
    }
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-semibold">Gérer les packs produits</h1>
        <div className="flex items-center gap-3">
          <SearchBar onChange={(e) => setSearch(e.target.value)} />
          <AddButton onClick={openAddModal}>+ Ajouter un pack</AddButton>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {filtered.map((pack) => (
          <div
            key={pack.id}
            className="bg-white rounded-2xl p-4 shadow border flex flex-col"
          >
            ...
            <div className="mt-4 flex-1">
              ...
              <div className="mt-3 flex items-center justify-between">
                <div>
                  ...
                  <p
                    className={`text-xs ${
                      pack.status ? "text-green-600" : "text-gray-400"
                    }`}
                  >
                ...
                <label className="relative inline-flex items-center cursor-pointer">
                  ...
                  <div
                    className={`w-12 h-6 rounded-full transition ${
                      pack.status ? "bg-green-500" : "bg-gray-300"
                    }`}
                  />
                </label>
              </div>
      ...
      {showEditModal && editingPack && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 px-4">
          <div className="bg-white rounded-2xl shadow-lg w-full max-w-md max-h-[90vh] flex flex-col p-6 relative overflow-y-auto">
            ...
            <div className="px-6 pb-6">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setEditingPack((p) => ({ ...p, name: e.target.value }))
                  }
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) =>
                    setEditingPack((p) => ({ ...p, description: e.target.value }))
                  }
                  className="mt-1 w-full border rounded-lg p-2"
                  rows={3}
                />
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 px-4">
          <div className="bg-white rounded-2xl shadow-lg w-full max-w-sm p-6 text-center">
            ...
            <p className="text-gray-500 mb-6">
              ...
              <span className="font-medium text-gray-800">{toDeletePack.name}</span> ?
            </p>
